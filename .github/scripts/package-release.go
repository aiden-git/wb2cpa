// Package-release zips a compiled plugin library at the archive root and
// writes a sha256sum-format checksum line. Used by the release workflow so
// CLIProxyAPI plugin-store installs accept the asset layout.
//
// Usage:
//
//	go run ./.github/scripts/package-release.go \
//	  -library dist/workbuddy.so \
//	  -archive workbuddy_0.3.3_linux_amd64.zip \
//	  -checksum workbuddy_0.3.3_linux_amd64.zip.sha256
package main
...
func packageLibrary(libraryPath, archivePath string) ([]byte, error) {
	...
	// Zip root must contain only the library named <pluginID>.{so|dylib|dll}.
